...
import { useLists } from '../contexts/ListContext'; // New Context
...
const LibraryView = ({ onSelectMovie }) => {
    // State
    const [activeTab, setActiveTab] = useState('watchlist'); // 'watchlist' | 'watched' | 'lists'
    ...
    useEffect(() => {
        ...
        const repairMovies = async () => {
            ...
            const batch = candidates.slice(0, 5); // Reduced batch size
        ...
        const timeout = setTimeout(repairMovies, 1000);
    ...
    const topGenres = useMemo(() => {
        if (!watched || !watched.length) return ALL_GENRES.filter(g => [28, 35, 18, 878, 27].includes(g.id));
        const counts = {};
        const cache = getCachedGenres();
        watched.forEach(m => {
            let ids = m.genre_ids;
            if (!ids || ids.length === 0) {
                if (cache[m.id] && cache[m.id].genres) ids = cache[m.id].genres.map(g => g.id);
    ...
    if (!user) {
        return (
            <div className="min-h-[70vh] flex flex-col items-center justify-center p-6 text-center">
                ...
                <p className="text-gray-400 mb-8 max-w-sm">Inicia sesión para guardar tus películas favoritas, calificarlas y crear listas personalizadas.</p>
        ...
        <div className="min-h-screen pb-24 px-4 pt-4">
            <div className="sticky top-20 z-30 bg-background/95 backdrop-blur-md py-2 -mx-4 px-4 border-b border-white/5 mb-6">
                ...
                <div className="flex p-1 bg-surface rounded-xl mb-4 relative overflow-hidden">
                    ...
                    <button onClick={() => setActiveTab('lists')} className={cn("flex-1 flex items-center justify-center gap-2 py-2.5 text-xs sm:text-sm font-semibold rounded-lg transition-all z-10", activeTab === 'lists' ? "text-white" : "text-gray-500 hover:text-gray-300")}>
                        <ListBulletIcon className="w-4 h-4" /> Listas
                    </button>
                    <motion.div
                        className="absolute top-1 bottom-1 w-[calc(33.33%-4px)] bg-surface-elevated rounded-lg shadow-sm border border-white/5"
                        initial={false}
                        animate={{ x: activeTab === 'watchlist' ? 4 : activeTab === 'watched' ? "100%" : "200%" }}
                        transition={{ type: "spring", stiffness: 300, damping: 30 }}
                    />
                </div>

                {/* FILTERS (ONLY FOR Movies, NOT Lists) */}
                {activeTab !== 'lists' && (
                    <div className="flex gap-3 animate-fade-in">
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                activeFilterCount > 0 ? "bg-primary text-white border-primary" : "bg-surface-elevated text-gray-400 hover:text-white"
                            )}
                        >
                            <AdjustmentsHorizontalIcon className="w-5 h-5" />
                            {activeFilterCount > 0 && (
                                <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full text-[10px] flex items-center justify-center font-bold text-white border border-[#121212]">{activeFilterCount}</span>
                            )}
                        </button>
                    </div>
                )}
                ...
                {activeTab !== 'lists' ? (
                    ...
                    <>
                        <div className="text-xs text-gray-500 font-medium uppercase tracking-wider flex justify-between items-center">
                            ...
                            {activeFilterCount > 0 && (
                                <button onClick={clearFilters} className="text-primary flex items-center gap-1 hover:underline"><XMarkIcon className="w-3 h-3" /> Limpiar filtros</button>
                            )}
                        </div>
                        ...
                        {totalCount === 0 ? (
                            <div className="flex flex-col items-center justify-center py-20 opacity-50">
                                ...
                                <p className="text-gray-400">
                                    {activeFilterCount > 0 ? "No hay coincidencias en esta lista." : activeTab === 'watchlist' ? "Tu lista está vacía." : "Aún no has marcado películas vistas."}
                                </p>
                                {activeFilterCount > 0 && <button onClick={clearFilters} className="mt-4 text-primary text-sm font-bold">Limpiar filtros</button>}
                            </div>
                        ) : (
                            <motion.div
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <button className="text-primary text-sm font-medium hover:underline flex items-center gap-1">
                                <PlusIcon className="w-4 h-4" /> Crear Nueva
                            </button>
                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {myLists.map(list => (
                                    <div
                                        ...
                                        className="group bg-surface-elevated border border-white/5 rounded-xl overflow-hidden hover:border-primary/50 transition-all cursor-pointer relative"
                                    >
                                        {/* Collage Cover */}
                                        <div className="h-32 grid grid-cols-4 bg-black/50 overflow-hidden relative">
                                            ...
                                            {(!list.movies || list.movies.length === 0) && (
                                                <div className="col-span-4 flex items-center justify-center bg-gray-900 h-full text-gray-700">
                                        ...
                                        <div className="p-4 relative bottom-0">
                                            ...
                                            <div className="flex items-center gap-3 text-xs text-gray-400">
                                                ...
                                                <div className="flex items-center gap-1">
                                                    {list.privacy === 'private' ? <LockClosedIcon className="w-3 h-3" /> : list.privacy === 'friends' ? <UserGroupIcon className="w-3 h-3" /> : <GlobeAltIcon className="w-3 h-3" />}
                                                    <span className="capitalize">{list.privacy === 'friends' ? 'Amigos' : list.privacy === 'private' ? 'Privada' : 'Pública'}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            {/* Filter Portal Code (Same as before) */}
            {createPortal(
                    ...
                    <div className="space-y-8 pb-8">
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-3">
                                <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest">{activeTab === 'watchlist' ? 'Calidad TMDB Mínima' : 'Tu Calificación Mínima'}</h4>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {topGenres.map(g => (<FilterChip key={g.id} label={g.name} isSelected={selectedGenres.includes(g.id)} onClick={() => toggleGenre(g.id)} />))}
                            </div>
                        </div>
